...
request.onupgradeneeded = function(event) {
    // save reference to the databse
    const db = event.target.result;
    // create an object store (table) called `new_pizza`, set to have auto incrementing primary key of sorts
    db.createObjectStore('new_pizza', { autoIncrement: true });
};

// upon a successful
request.onsuccess = function(event) {
    // when db is successfully created with its object store (from ononupgradedneeded event above) or simply established a connection, save reference to db in global variable
...
// executed if we attempt to submit new pizza and there's no connection
